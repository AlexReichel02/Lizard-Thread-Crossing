...
mutex lock1; // AR
int numLizardsCrossed=0; //AR
 sem_t lizardCross; // AR
 ...
 */
 ...
 // launch the thread to simulate the cat's behavior
	 _catThread = new thread (catThread, this); // AR
 }
...
if (debug)
    {
cout << "Cat[" << _id << "] sleeping for " << sleepSeconds << " seconds" << endl;
cout << flush;
...
//cout<<"Cat: " <<  aCat->_id << " Is awake and watching" <<endl; //AR



cout<<numLizardsCrossed << " lizards crossed in total" << endl; //AR

if (numCrossingSago2MonkeyGrass + numCrossingMonkeyGrass2Sago >
MAX_LIZARD_CROSSING)
{

	cout<<numCrossingSago2MonkeyGrass << " <- going forward ,  going -> backwards " << numCrossingMonkeyGrass2Sago<<endl; //AR
	cout<<numLizardsCrossed << " lizards crossed in total" << endl;  //AR
...
if (debug)
    {
      cout << "Lizard[" << _id << "] sleeping for " << sleepSeconds << " seconds\n";
     cout << flush;

    }
sleep( sleepSeconds );
if (debug){
      cout << "Lizard[" << _id << "] awake \n";
      cout << flush;
...
{
	sem_wait(&lizardCross);
if (debug)
    {
cout << "Lizard[" << _id << "] checking  sago -> monkey grass\n";
cout << flush;

    }

//sem_post(&lizardCross);


if (debug)
    {
cout << "Lizard[" << _id << "] thinks  sago -> monkey grass  is safe\n";
...
if (debug)
    {
      cout << "Lizard[" << _id << "] crossing  sago -> monkey grass\n";
      cout << flush;
...
void Lizard::madeIt2MonkeyGrass()
{
	...
	sem_post(&lizardCross);
		...
		if (debug)
			    {
			cout << "Lizard[" << _id << "] made the  sago -> monkey grass  crossing" <<endl;
			cout << flush;
...
if (debug)
    {
cout << "Lizard[" << _id << "] eating for " << eatSeconds << " seconds" <<
endl;
cout << flush;
...
if (debug)
    {
      cout << "Lizard[" << _id << "] finished eating" << endl;
     cout << flush;
...
if (debug)
    {
cout << "Lizard[" << _id << "] checking  monkey grass -> sago" << endl;
cout << flush;
...
if (debug)
    {
cout << "Lizard[" << _id << "] thinks  monkey grass -> sago  is safe" <<
endl;
cout << flush;
...
if (debug)
    {
cout << "Lizard[" << _id << "] crossing  monkey grass -> sago" << endl;
cout << flush;
    }
    /*
     * One more crossing this way
     */
lock1.lock(); //AR
numCrossingMonkeyGrass2Sago++; //AR
lock1.unlock(); //AR
    /*
     * Check for lizards cross both ways
     */
...
if (debug)
	...
	lock1.lock(); //AR
	numLizardsCrossed++; //AR
	lock1.unlock(); //AR
cout << "Lizard[" << _id << "] made the  monkey grass -> sago  crossing" <<
endl;
cout << flush;
...
if (debug)
    {
      cout << aLizard->getId() << " lizard is alive \n";
    //  cout << flush<<endl;
    }
while(running)
...
/*
     ...
     */
	 ...
	 sem_init(&lizardCross, 0,MAX_LIZARD_CROSSING);
...
/*
     ...
     */
         ...
         vector<Cat*> allCats;
          for (int i=0; i < NUM_CATS; i++) {   //AR
           allCats.push_back(new Cat(i));    //AR
             }      						//AR
...
for (int i=0; i < NUM_LIZARDS; i++) { //AR
          allLizards[i]->wait();     //AR
      }
/*
